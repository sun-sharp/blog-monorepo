...
const checkStatus = (statusCode: number, msg: string): void => {
  switch (statusCode) {
    ...
    case 401:
      ...
      break;
    case 403:
      showToast(msg || '访问被禁止！', 'error');
      break;
    case 404:
      showToast(msg || '未找到该资源！', 'error');
      break;
    case 500:
      showToast(msg || '服务器错误！', 'error');
      break;
    default:
      showToast(msg, 'error');
  }
...
export class CustomRequest {
  ...
  request<T = any>(config: CustomRequestConfig): Promise<T> {
    ...
    return new Promise<T>((resolve, reject) => {
      ...
      uni.request({
        ...requestConfig,
        success: (res) => {
          const result = this.processResponse(res);
  ...
  private processRequestConfig(config: Omit<CustomRequestConfig, 'requestOptions' | 'responseOptions'>) {
    ...
    if (completeToken) {
      headers.Authorization = completeToken;
    }
    ...
    if (!isEmpty(params)) {
      ...
      const queryString = Object.entries({ ...params, ...timestampParams })
  ...
  private async processResponse(res: UniApp.RequestSuccessCallbackResult): Promise<any> {
    const {
      isShowMessage = true,
      isShowErrorMessage,
    ...
    if (statusCode !== 200) {
      ...
      throw new Error(msg || `请求错误 ${statusCode}`);
    }
    ...
    if (isShowMessage) {
      if (hasSuccess && (successMessageText || isShowSuccessMessage)) {
        showToast(successMessageText || message || '操作成功！', 'success');
      } else if (!hasSuccess && (errorMessageText || isShowErrorMessage)) {
        showToast(message || errorMessageText || '操作失败！', 'error');
