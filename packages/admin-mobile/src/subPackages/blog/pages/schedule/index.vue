<template>
  <view class="page">
    <view class="search-bar">
      <u-search v-model="keyword" placeholder="搜索日程" @search="onSearch" @custom="onSearch" />
    </view>

    <scroll-view scroll-y class="list-scroll" @scrolltolower="onLoadMore">
      <view class="list-item" v-for="item in dataList" :key="item.scheduleId">
        <view class="item-main">
          <view class="item-info">
            <text class="item-title">{{ item.title }}</text>
            <text class="item-desc">{{ item.startDate }} {{ item.startTime }}-{{ item.endTime }}</text>
          </view>
        </view>
...
const fetchData = async (reset = false) => {
  ...
  try {
    const res = await scheduleAPi.getFindPage({ current: current.value, size: DEFAULT_PAGESIZE, keywords: keyword.value || undefined });
...
const onRemove = (item: any) => {
  uni.showModal({ title: '提示', content: `确定删除日程 ${item.title} 吗？`, success: async (res) => { if (res.confirm) { await scheduleAPi.remove(item.scheduleId); fetchData(true); } } });
};
