<template>
  <view class="page">
    <view class="search-bar">
      <u-search v-model="keyword" placeholder="搜索支付宝账单" @search="onSearch" @custom="onSearch" />
    </view>

    <scroll-view scroll-y class="list-scroll" @scrolltolower="onLoadMore">
      <view class="list-item" v-for="item in dataList" :key="item.aliPayId">
        <view class="item-main">
          <view class="item-info">
            <text class="item-title">{{ item.tradeOtherPerson || item.productDescription }}</text>
...
const fetchData = async (reset = false) => {
  ...
  try {
    const res = await aliPayApi.getPage({ current: current.value, size: DEFAULT_PAGESIZE, tradeOtherPerson: keyword.value || undefined });
