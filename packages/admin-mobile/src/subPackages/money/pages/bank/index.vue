<template>
  <view class="page">
    <view class="search-bar">
      <u-search v-model="keyword" placeholder="搜索银行账单" @search="onSearch" @custom="onSearch" />
    </view>

    <scroll-view scroll-y class="list-scroll" @scrolltolower="onLoadMore">
      <view class="list-item" v-for="item in dataList" :key="item.bankId">
        <view class="item-main">
          <view class="item-info">
            <text class="item-title">{{ item.tradeOtherPerson || item.explain }}</text>
            <text class="item-desc">{{ item.tradeTime }}</text>
          </view>
...
const fetchData = async (reset = false) => {
  ...
  try {
    const res = await bankApi.getPage({ current: current.value, size: DEFAULT_PAGESIZE, tradeOtherPerson: keyword.value || undefined });
