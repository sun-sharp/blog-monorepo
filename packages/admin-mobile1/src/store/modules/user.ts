...
export const useUserStore = defineStore({
  ...
  getters: {
    ...
    getUserInfo(): ApiUserInfo { return this.info; },
    getConfigInfo(): ApiConfigInfo { return this.configInfo; },
  },
  actions: {
    async login(loginForm: ApiCapitalLoginData) {
      ...
      if (token) { this.token = token; storage.set(ACCESS_TOKEN, token); }
      return res;
    },
    ...
    async GetConfigInfo(): Promise<ApiConfigInfo> {
      ...
      const result: ApiConfigInfo = { navMode: configInfo.navMode, theme: configInfo.theme, headerDark: configInfo.headerDark, siderDark: configInfo.siderDark, tabs: configInfo.tabs, footer: configInfo.footer, breadcrumb: configInfo.breadcrumb, pageAnimate: configInfo.pageAnimate, pageAnimateType: configInfo.pageAnimateType };
      this.configInfo = result;
      storage.set(USER_CONFIG, result);
      return result;
    },
    logout() {
      this.token = ''; this.info = defaultUserInfo; this.configInfo = {} as ApiConfigInfo;
      storage.remove(ACCESS_TOKEN); storage.remove(CURRENT_USER); storage.remove(USER_CONFIG); storage.remove(IS_LOCK_SCREEN);
    },
  },
});

export function useUserStoreWidthOut() { return useUserStore(); }
