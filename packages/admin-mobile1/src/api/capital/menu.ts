...
export const getMenuList = (): Promise<ApiMenuItem[]> => {
  return RequestCapital.request({ url: `${basic}/find_term`, method: 'GET' });
};
...
export const removeMenu = (menuId: string): Promise<undefined> => {
  return RequestCapital.request({ url: `${basic}/${menuId}`, method: 'DELETE', responseOptions: { isShowSuccessMessage: true } });
};
