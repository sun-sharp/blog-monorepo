...
export const certainTypeAll = (type: string): Promise<ApiCategoryItem[]> => {
  return RequestCapital.request({ url: `${basic}/certain_type_all`, method: 'GET', params: { type } });
};
...
export const remove = (categoryId: string): Promise<undefined> => {
  return RequestCapital.request({ url: `${basic}/${categoryId}`, method: 'DELETE', responseOptions: { isShowSuccessMessage: true } });
};
