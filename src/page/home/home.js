...
import { Avatar, Carousel, Input, Layout, Select } from 'antd';
...
class Home extends Component {
	...
	getBanner = () => {
		...
		axios.get(API.BANNER_FIND, { params }).then(({ data }) => {
			if (data.code === 0) {
	...
	getArticleList = (keyword) => {
		...
		const postData = {
			title: keyword,
			categoryVal: self.state.classifyValue,
			current: 1,
			size: 999
		};
		axios.post(API.ARTICLE_FIND, postData).then(({ data }) => {
			if (data.code === 0) {
				console.log(data);
	...
	getClassify = () => {
		const self = this;
		axios.get(API.CATEGORY_FIND_ALL).then(({ data }) => {
			if (data.code === 0) {
				const listData = (data.result || []).map((m) => ({
					title: m.name,
					value: m.value
				}));
				listData.unshift({
	...
	render() {
		...
		this.state.articleList.forEach((item) => {
			// 将简介中加入省略号
			const brief =
				item.brief && item.brief.length > 100
					? `${item.brief.slice(0, 100)}...`
					: item.brief || '';
			// 处理连接跳转
			const linkTo = `/articleDetails?articleId=${item.articleId}`;
			const image = '';
			// 处理循环样式
			articleItem.push({
				title: item.title,
				...
				{!isShowSlider ? (
					...
					<Sider width={330} className="home-slider">
						...
						<div className="slider-about">
							...
							<div className="slider-about--text">
								{' '}
								一个90后草根站长！18年入行。一直潜心研究web前端技术，一边工作一边积累经验，分享一些个人工作的经历，以及遇到的问题和解决方法。
							</div>
						</div>
						{/*搜索*/}
