    ...
    <n-form ref="modelFromRef" class="we-chat-update-model__body" :model="modelForm" :rules="modelRules" label-placement="left" :label-width="120">
      ...
      <n-form-item label="流入/流出" path="inflowOrOutflow">
        <n-radio-group v-model:value="modelForm.inflowOrOutflow" name="radiogroup">
          <n-space>
            <n-radio v-for="item in weChatInflowOrOutflowOption" :key="item.value" :value="item.value">
              {{ item.label }}
            </n-radio>
          </n-space>
        </n-radio-group>
      ...
      <n-form-item label="账单类型" path="billType">
        <n-select v-model:value="modelForm.billType" filterable :options="weChatBillTypeOption" placeholder="请选择账单类型" />
      </n-form-item>
    </n-form>
...
<script lang="ts">
  import { weChatBillTypeOption, weChatInflowOrOutflowOption } from '@/constant';
  ...
  export default defineComponent({
    ...
    setup(_props, { emit }) {
      ...
      return {
        ...
        formBtnLoading,
        roleOption,
        weChatInflowOrOutflowOption,
        weChatBillTypeOption,
        init,
        confirmForm,
      };
    },
