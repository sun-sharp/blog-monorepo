...
// 设置粒子背景
const setupParticleBack = (window: browserWindow & typeof globalThis) => {
  ...
  const createParticle = () => {
    let canvas = document.getElementById('particleId') as HTMLCanvasElement;
    if (!canvas) {
  ...
  const init = () => {
    const { body } = document;
    const { canvas, ctx } = createParticle();
  ...
  // 销毁
  const clear = () => {
    const canvas = document.getElementById('particleId') as HTMLCanvasElement;
...
// 初始化
export const initParticleBack = () => {
  const particleBack = setupParticleBack(window);
  particleBack.init();
};

// 销毁
export const clearParticleBack = () => {
  const particleBack = setupParticleBack(window);
  particleBack.clear();
};
