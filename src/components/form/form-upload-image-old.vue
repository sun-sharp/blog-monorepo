  ...
  <div class="w-full">
    <div class="upload">
      <div class="upload-card">
        <!--图片列表-->
        <div v-for="(item, index) in imgList" :key="`img_${index}`" class="upload-card-item" :style="getCSSProperties">
          <div class="upload-card-item-info">
            ...
            <div class="img-box-actions">
              ...
              <n-icon v-if="showRemoveButton" size="18" class="action-icon mx-2" @click="remove(index)">
                <DeleteOutlined />
              </n-icon>
            </div>
          </div>
...
<script lang="ts">
  ...
  import { EyeOutlined, DeleteOutlined, PlusOutlined, getUploadAction, getImgUrl, isString } from '@/utils';
  import { useMessage, useDialog, NUpload } from 'naive-ui';
  import { useUserStoreWidthOut } from '@/store/modules/user';
  import { componentUpload, ResultEnum } from '@/constant';
  ...
  export default defineComponent({
    ...
    setup(props, { emit }) {
      ...
      function checkFileType(fileType: string) {
        return componentUpload.fileType.includes(fileType);
      }
      ...
      function beforeUpload({ file }) {
        ...
        // 设置类型,则判断
        const fileType = componentUpload.fileType;
