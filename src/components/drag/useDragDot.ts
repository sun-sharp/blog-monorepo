...
import { IDragData } from '/#/views/dashboard';
...
export const useDragDot = (dragData: Ref<IDragData>, props: ExtractPropTypes<typeof DragProps>, resizeEmitFun: Function) => {
  ...
  const getNewStyle = (type: string, rect: any, deltaW: number, deltaH: number, ratio: number | undefined, minWidth: number, minHeight: number) => {
  ...
  const centerToTL = ({ centerX, centerY, width, height }: any) => ({
    top: centerY - height / 2,
    left: centerX - width / 2,
    width,
    height,
  });
  const formatData = (data, centerX: number, centerY: number) => {
  ...
  // 缩放
  const onDotMousedown = (e: MouseEvent, dotInfo: any) => {
    e.stopPropagation();
    e.preventDefault();
    // 获取鼠标按下的坐标
